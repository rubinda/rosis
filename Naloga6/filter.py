...
b = np.array([8, 3, 1/9, 0, 1, 7/11, 16/19])  # b koeficienti filtra (nicle)
a = np.array([3/4, 1, 1/3, 6/13, 25/29, 3, 3/8])  # a koeficient filtra (poli)


def plot_freq_response(h, filename, fignum=2):
    """ Shrani png frekvencnega odziva v datoteko"""
    plt.figure(fignum)
    plt.plot(np.real(h), label='Frekvenčni odziv')
...
plot_freq_response(h, 'images/freq-response_before.png', fignum=2)
...
plot_freq_response(h, 'images/freq-response_after.png', fignum=5)
